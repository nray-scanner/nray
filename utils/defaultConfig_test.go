...
func TestApplyDefaultTargetgeneratorCertificatetransparencyConfig(t *testing.T) {
	var result *viper.Viper

	// Test passing nil to the function
	result = utils.ApplyDefaultTargetgeneratorCertificatetransparencyConfig(nil)
	if !result.IsSet("enabled") || result.GetBool("enabled") != false {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("domainRegex") || result.GetString("domainRegex") != "^.*$" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("tcpports") {
		t.Errorf("Test failed: Passing nil to config")
	...
	// Test passing an empty viper to the function
	emptyViper := viper.New()
	result = utils.ApplyDefaultTargetgeneratorCertificatetransparencyConfig(emptyViper)
	if !result.IsSet("enabled") || result.GetBool("enabled") != false {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("domainRegex") || result.GetString("domainRegex") != "^.*$" {
	...
	// Pass a viper with a value explicitly set. The value mustn't change.
	viperWithValue := viper.New()
	viperWithValue.Set("domainRegex", "^(www[.]).*([.]com)$")
	viperWithValue.Set("enabled", true)
	viperWithValue.Set("maxHostsPerBatch", 100)
	viperWithValue.Set("maxTcpPortsPerBatch", 50)
	viperWithValue.Set("maxUdpPortsPerBatch", 0)
	result = utils.ApplyDefaultTargetgeneratorCertificatetransparencyConfig(viperWithValue)
	if !result.IsSet("enabled") || result.GetBool("enabled") != true {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("domainRegex") || result.GetString("domainRegex") != "^(www[.]).*([.]com)$" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("tcpports") {
		t.Errorf("Test failed: Passing nil to config")
...
func TestApplyDefaultTargetgeneratorLDAPConfig(t *testing.T) {
	var result *viper.Viper

	// Test passing nil to the function
	result = utils.ApplyDefaultTargetgeneratorLDAPConfig(nil)
	if !result.IsSet("enabled") || result.GetBool("enabled") != false {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapSearchString") || result.GetString("ldapSearchString") != "(objectCategory=computer)" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("baseDN") || result.GetString("baseDN") != "dc=contoso,dc=com" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapAttribute") || result.GetString("ldapAttribute") != "dNSHostName" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapServer") || result.GetString("ldapServer") != "" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapPort") || result.GetUint("ldapPort") != 636 {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("insecure") || result.GetBool("insecure") != false {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapUser") || result.GetString("ldapUser") != "" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapPass") || result.GetString("ldapPass") != "" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("tcpports") {
		t.Errorf("Test failed: Passing nil to config")
	...
	// Test passing an empty viper to the function
	emptyViper := viper.New()
	result = utils.ApplyDefaultTargetgeneratorLDAPConfig(emptyViper)
	if !result.IsSet("enabled") || result.GetBool("enabled") != false {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapSearchString") || result.GetString("ldapSearchString") != "(objectCategory=computer)" {
	...
	viperWithValue := viper.New()
	viperWithValue.Set("enabled", true)
	viperWithValue.Set("baseDN", "dc=nray-scanner,dc=org")
	viperWithValue.Set("ldapServer", "nray-scanner.org")
	viperWithValue.Set("ldapPort", 389)
	viperWithValue.Set("insecure", true)
	viperWithValue.Set("maxHostsPerBatch", 100)
	viperWithValue.Set("maxTcpPortsPerBatch", 50)
	viperWithValue.Set("maxUdpPortsPerBatch", 0)
	result = utils.ApplyDefaultTargetgeneratorLDAPConfig(viperWithValue)
	if !result.IsSet("enabled") || result.GetBool("enabled") != true {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapSearchString") || result.GetString("ldapSearchString") != "(objectCategory=computer)" {
	...
	if !result.IsSet("baseDN") || result.GetString("baseDN") != "dc=nray-scanner,dc=org" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapAttribute") || result.GetString("ldapAttribute") != "dNSHostName" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapServer") || result.GetString("ldapServer") != "nray-scanner.org" {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapPort") || result.GetUint("ldapPort") != 389 {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("insecure") || result.GetBool("insecure") != true {
		t.Errorf("Test failed: Passing nil to config")
	}
	if !result.IsSet("ldapUser") || result.GetString("ldapUser") != "" {
		t.Errorf("Test failed: Passing nil to config")
